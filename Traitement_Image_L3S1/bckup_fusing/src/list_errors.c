...
int initialize_firstlast(ErrorList_FirstLast *firstlast_list) {
	firstlast_list->first = NULL;
	firstlast_list->last = NULL;
	return 1;
}
...
int add_front_list(ErrorList *first, Tree *leaf, Zone zone) {
...
int add_end_list(ErrorList *last, Tree *leaf, Zone zone) {
	ErrorListElement *new = NULL;
